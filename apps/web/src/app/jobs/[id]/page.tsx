...
export default function JobDetailPage() {
  ...
  const [editingClipUrl, setEditingClipUrl] = useState<string | null>(null);
  ...
  async function handleEdit(clip: Clip) {
    // Fetch video URL for preview in editor
    let url: string | null = clipUrls[clip.id] || null;
    if (!url) {
      try {
        const res = await fetch(`/api/clips/${clip.id}/download`);
        if (res.ok) {
          const data = await res.json();
          url = data.url;
          setClipUrls((prev) => ({ ...prev, [clip.id]: data.url }));
        }
      } catch {
        // Continue without preview
      }
    }
    setEditingClipUrl(url);
    setEditingClip(clip);
  }
                ...
                <div key={clip.id} className="glass-card overflow-hidden group" id={`clip-${clip.id}`}>
                  ...
                  <div className="p-4">
                    ...
                    {clip.status === "finalizing" && (
                      <div className="flex items-center gap-2 mt-3 text-xs text-accent-2">
